...
<!-- title Bar -->
	<div id="title_subcat" class="bodbot">
		<?php
			include 'category/categoryPageTitle.php';
		?>
	</div>
	<div id="sub_cats" class="bodbot">
		<div class="down_double right cursor" id="expand_sub_category" style="display: none;"></div>
		<ul>
		<?php
			include 'category/subCategoryList.php';
		?>
		</ul>
	</div>
<!-- title Bar ends -->
...
<!-- Hot Content -->
	<div id="top_drops" style="margin-right: 10px;">
		...
		<?php
			include 'product/subCategoryDrops.php';
		?>
	</div>
<!-- Hot Content ends -->
...
<!-- 	<script type="text/javascript" src="js/newsrotate.js"></script> -->
	...
	<script type="text/javascript">
		...
		function pageSpecificFunctions() {
			...
			/* Sub category-list expand/collapse */
			subCategoryListExpandCollapse();
		}
		function pageSpecificResize() {
			/* Chirag: Commented as there is no price_alert box for now */
			// $("#pricealert_thumb").width($("#pricealert_thumb").parent().innerWidth() - 75);

			/* Resizing changes the number of sub-categories shown in one line, 
			so handling that. */
			subCategoryListExpandCollapse_resize();
