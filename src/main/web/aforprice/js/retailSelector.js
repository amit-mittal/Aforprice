 ...
 * 
 * It is a plugin to jQuery to control the
 * retailer selection used in the website.
 * */
(function($){
	...
	var retailSelector = function(elem, userOptions){
		var defaults = {
				...
				defaultRetailer: "toysrus",
				scrollSpeedPix: 5,
				scrollSpeedTime: 10,
				
				selectionCallback: Categories.loadCategoriesMenu,
		...
		var setStyles = function() {
			...
			// Change the url with new default retailer
			Url.pushStateSearch('retailer', options.defaultRetailer);
			
			if(!options.multipleSelection) {
				selectedRetailer.parent();
				stickySelected = selectedRetailer.clone();
				//add sticky class attribute and put this in front of all retailer logo section
		...
		//this is called when user selects/deselects a retailer's logo
		toggleRetailer = function() {
			if(stickySelected)
				stickySelected.remove();
			//flip the selection of this retailer
...
//				console.log("change: "+change+", left"+left);
		...
		//bind different functions to mouse events on various components
		bindEvents = function() {
			if(options.multipleSelection)
